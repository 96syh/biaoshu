...
3. 替换项目变量时，只替换历史项目名称、招标人/业主名称、历史日期、地点、金额、服务期限、承诺时限等当前项目冲突事实。
4. 人员、职称、证书编号、联系方式、设备、软件、资质等企业固定资料如果已在 matched_blocks 或 reference_text 中出现，视为历史库可复用资料；除非当前招标要求明确冲突，不要替换成 〖待补充〗。
5. 历史库已有人员表、设备表、证书表或业绩表时，默认保留表格和原始块位置，只按当前评分项补充说明或替换项目变量。
6. 需要补评分项时，只追加最少必要语句，不扩写通用方案；优先 insert_after 到最相关原文块后。
7. 需要删除内容时，只删除与当前章节无关、明显冲突或违反暗标/格式要求的文字，不删除整张表或图片块，除非该块整体不属于当前章节。
8. 图片位置分级处理：
...
9. 暗标章节不得新增投标人名称、人员姓名、联系方式、Logo、商标、可识别案例名或图片。
"""
...
历史正文主稿规则：
1. history_reference_drafts 是正文生成的主依据。优先使用第一个 high/medium 匹配的 reference_text，保留它的篇幅、段落顺序、标题层级、表格、图片、图注、附件占位和清单结构。
...
4. 历史正文中的 Markdown 表格、HTML 表格、图片语法、图片占位、图注和素材引用必须原样保留位置；如图片事实不确定，只改图注中的项目名，不删除图片块。
