...
6. 新增或替换的文字必须像正式投标正文，不得写成审校意见、核验提示、材料清单提醒、后台操作说明或面向编制人员的备注。
...
12. 严禁在正文中输出 HTML 表格标签，包括 <table>、<tr>、<th>、<td>、<thead>、<tbody>。如果需要表格，必须直接输出 Markdown 表格
13. 如果表格上方需要补一句说明，该说明必须写成正式表述，例如“拟投入本项目的主要人员情况见下表”“相关证明材料按招标文件要求附后”，不要写“按评分项统计口径”“核验为准”“缺失时补齐”“页码待编排”“详见附件”等审校式口吻。
14. 只有“全部响应、无偏离”而没有具体条款、对象、响应内容或证明材料的空响应表不得新增、保留或重建；可用一句“我公司完全响应招标文件相关要求”替代。
15. 项目负责人只有 1 人时，不要为项目负责人单独生成表格；用一段文字说明姓名、职称、资格和职责。项目组成人员表有几人，正文就只能围绕这几人写，禁止写“110余人、若干专业工程师”等与表格行数不一致的笼统人数。
"""
...
7. 正文语气必须像正式投标文件成稿，不得写成审校说明、核验口径说明、资料催办提醒、系统注释、编制备注或面向内部人员的操作提示。
...
5. 不得把表格的表头和单元格内容压平成普通段落连续输出；表格、图片、流程图由后端 Word/HTML 块或素材规划承载，正文只写必要的引导句和解释句。
6. 禁止输出类似“按评分项统计口径”“以核验为准”“缺失扫描件按……补齐”“详见相应附件”“确认是否达到”“待编制人员补充”等审校式或办文式表达；如确需表达附件、证明材料或未定信息，必须改写成正式投标语言。
...
3. 表单、承诺、偏离、价格、资格、材料附件等章节按正式投标文件口吻写作，优先输出成稿化说明、表前引导语和必要承诺，不输出审校提示或办理说明。
4. 组织机构、设备软件、图片证书、截图案例等只能依据企业资料或素材库；缺失时采用克制、正式的保守表述或最小占位，不写成资料催办语气。
5. 样例 profile 只迁移段落顺序、句式骨架、表格/图片位置和风格；不得照抄样例原句或继承样例事实。
6. 与同级章节避免重复；高分、阻塞风险和证据链章节写细，低风险说明性章节简洁准确。
7. 同一自然段避免连续重复“包括、开展、配合、确保”等句式；涉及清单时拆成完整短句，保证读者能按句读懂，而不是堆叠关键词。
8. 表格上方如需引导语，应简短、正式、面向评审阅读，例如“主要人员配置情况见下表”“拟投入的仪器设备情况如下”；不要写核验规则、统计口径、附件追踪或内部检查要求。
9. 若本章是“项目负责人”，且只涉及 1 名负责人，应使用正文段落说明，不生成项目负责人表或重复业绩响应表；若本章是“项目组成人员详情”，正文描述的人数、姓名、岗位和职责必须与表格中的人员逐行一致，不能写多于表格行数的团队规模。
10. 不要生成只有表头、空单元格或仅含“全部响应、无偏离”的响应情况表；没有具体响应对象时，用正式响应句替代表格。
11.严禁在正文中输出 HTML 表格标签，包括 <table>、<tr>、<th>、<td>、<thead>、<tbody>。如果需要表格，必须直接输出 Markdown 表格
12.章节开头去机械化：除“服务范围、项目概况、总体说明”等确需交代背景的章节外，首段不得反复堆叠完整项目名称、招标人、服务地点、服务期限、费用限额和服务范围；优先使用“本项目”“本次服务”“该项工作”等简洁指代。若历史块开头存在“本项目服务对象为……”“本项目服务内容围绕……”“我公司将围绕……项目特点”“我公司针对……项目建立……”等机械套句，应改成直接切入当前章节主题的表述。
历史正文主稿规则：
...
5. 占位只在确实无法避免时使用，且应尽量短；除占位本身外，不要再追加“缺失说明”“核验提醒”“补齐提示”“附件追踪说明”等审校化文字。
